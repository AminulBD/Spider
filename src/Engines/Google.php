...
class Google extends Base
{
    ...
    protected ?string $baseUri = 'https://www.google.com/';
    ...
    private function parseContent(string $html): void
    {
        ...
        $crawler->filter('#rso div .g')->each(function (Crawler $node) {
            try {
                $link = $node->filter('[data-snhf="0"] a, div.g a')->first();
                $title = $node->filter('[data-snhf="0"] a > h3, div.g a > h3')->first();
                $description = $node->filter('[data-sncf="1"] > div, div.g > div > div > div:nth-child(2)')->first();
                $siteName = $node->filter('[data-snhf="0"] a > div > div > span, div.g a > div > div > span')->first();
